...
1. The endpoint rule registry: the config schema, signed and versioned, plus the matcher (SNI and host and path predicates, first-match). Pure and unit-testable.
2. Explicit forward-proxy interception for the no-MITM surfaces: `HTTPS_PROXY` plus base-URL pinning for agents and IDEs, applying `inspect-prompt` and `govern-tool-call` by reusing the gateway and proxy decision paths.
3. TLS-interception mode (ACP CA) for body-inspecting rules on managed devices, with the cert-pinning limits documented and detected (a pinned endpoint is reported, not silently failed).
