...
4. The browser surface: an extension or PAC-based path. DONE: `acp intercept pac` (a browser/OS PAC that routes governed endpoints through the proxy) and `acp intercept extension` (a generated Chromium MV3 extension that installs the PAC and badges governed tabs).
