...
- [x] **H0.3 KMS/HSM signing + rotation [2].**
  - [x] `acp_core::keymgr` (LocalKms rotation, tested) + `acp-hsm`: real PKCS#11 HSM Ed25519 signer, VERIFIED against SoftHSM (HSM signature verifies with the ledger's verify_ed25519 path, raw 32B pubkey / 64B sig). Real HSM = ACP_PKCS11_MODULE swap. `acp_hsm::ThreadedPkcs11Signer` is the Send handle (dedicated signing thread) the ledger takes; verified against SoftHSM (signs across a thread boundary as Box<dyn Signer + Send>).
...
- [m] **v2.4.1 IdP-backed authority + app/agent registry.**
  - [x] Principal + roles from a verified Entra token (`acp-auth`); `acp-registry` (apps/agents, one-time token, immediate revocation) WIRED into the proxy: `--registry/--agent-id/--agent-token` verify the presented credential (fail-closed: a bad/revoked token refuses to start), stamp the verified app/agent NAME into the trusted policy context, and record the verified agent in evidence. Policy DSL `app`/`agent` matchers enforce per-app/per-agent LIVE (E2E: triage denied charge_card, analytics allowed, bad token fails closed). CLI `acp app/agent/registry`. Addresses the D10 self-asserted-identity gap for agents.
...
- [~] **H2.1 Large-ledger performance [9].**
  - [x] Inclusion-proof cost validated logarithmic (merkle_invariants.rs) + `acp bench-ledger <n>` load tool times append/verify throughput; archival/compaction = tiering doc + the archive-only verify test. [x] append is now O(n log n) (incremental Merkle frontier) + synchronous=NORMAL + single-transaction append: ~12.5k appends/sec and ~22k verify/sec in release (20k in 1.6s/0.9s, was O(n^2)); [ ] the full 100M run is `bench-ledger` on a load box.
...
- [~] **X.6 Docs + deprecation policy [R7].**
  - [x] Deprecation/support-window policy documented (docs/ops/deprecation-policy.md); signed, versioned policy DEPLOYMENT: `acp policy deploy` validates+signs+versions into a store, proxy `--policy-dir` loads the current signed policy (verifies signature, fail-closed on tamper/bad-sig) and HOT-RELOADS on change (tested: v1 deny -> hot-deploy v2 -> same call forwarded, no restart). [ ] hosted docs site is infra.
...
- [x] **F2 [H0/P0] Break-glass / emergency controls.**
  - [x] `acp_core::breakglass` + `BreakGlassRegistry` wired into the proxy `Controller`: `engage_break_glass` applies active grants to the verdict in `decide_frame` (no-op by default), re-deriving enforcement; lockdown denies an otherwise-allowed call on the hot path (bg_tests; all existing enforcement tests still pass). [ ] the admin transport that calls engage is the deploy step.
