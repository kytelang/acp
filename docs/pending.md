...
- Phasing (from the design doc):
  1. Introduce the `Scorer` seam and refactor the current engine into `SignatureScorer` behind it. Pure refactor, zero behaviour change, no models needed. Safe first step. DONE (`acp_core::content`: Scorer/Signal/ContentEngine/verdict_from; scan_text unchanged; full workspace suite green).
...
For contrast, so the pending list is read against the whole. All of the following are built and tested (the full workspace suite passes, 0 failures):
...
7. HTTP/2 in the interception proxy: ACCEPTED LIMITATION (documented). Fail-closed on h2-only clients (recorded, not silently passed); HTTP/1.1 works. Workaround: base-URL pinning for agents/IDEs. Full h2 MITM is a future enhancement.
8. HA / DR: code in place (shared state via acp-pgstate verified; /healthz + /readyz on control plane and PEPs; acp ledger-backup). Replication / VIP failover / object-storage backup are deployment (p2-operations.md).
